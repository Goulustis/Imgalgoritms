...
public:
   ...
   * You may want a recursive helper function for this one.
   ...
   void prune(double tol);
	...
	//get average entropy 4 square 
	double aveEntrpy(stats & s, pair<int,int> SE, int k);
	
	// get lower right coordinate of given square
	pair<int,int> getCoordlr(pair<int,int> p, int k);
	
	//get the SW NE NW upper left coordinate; verctor = [SW NE NW];
	vector<pair<int,int>> getSplitCoord(pair<int,int> SE, int k);
	
	
	//get entropy of single square
	double getindvEntropy(stats & s, pair<int,int> p, int k);
	
	//fix coordinate and return pair
	pair<int,int> fixCoord(int x, int y, int k);
	
	vector<int> addvec(vector<int> vec1, vector<int> vec2);
	
	//generate next square img
	
	void putSqrImg(pair<int,int> p, PNG & putIm, PNG & oriImg);
	
	//helper to generate individual squares
	void genSqrImg(pair<int,int> p, int k, PNG & putImg, PNG & oriImg);
	
	
	//putting pixel of oriImg into toPut
	void putImgRender(PNG & toPutImg, PNG & oriImg, pair<int,int> p, int k);

	//helper for prune
	void prune(Node* root,double tol);

	//helper check tolerance
	bool checkTol(double tol, Node* node, HSLAPixel avg);
...
private:
   ...
   Node* root; // ptr to the root of the toqutree
	...
	// ============= helper function ============
	void renderPatch(Node * n, PNG & img);
