                       ...
                       double tolerance, int frameFreq)
{
     ...
     */
	 ...
	 for(int i = 0; i< w; i++){
		 ...
		 yvec.reserve(h);
		 for(int j = 0; j<h;j++){
			 yvec.push_back(false);
		 }
		 
		 marker.push_back(yvec);
	 }
	...
	// for(int j = 0; j<img.height();j++){
	 ...
	 while (! os.isEmpty()){
		 ...
		 //upright
		 process(img, px + 1, py - 1, fillColor, tolerance, frameFreq, dh, os, marker, numfram, ctr);
		 //up 
		 process(img, px, py - 1, fillColor, tolerance, frameFreq, dh, os, marker, numfram, ctr);
		 //upleft
		 process(img, px - 1, py - 1, fillColor, tolerance, frameFreq, dh, os, marker, numfram, ctr);
		 //left
		 process(img, px - 1, py, fillColor, tolerance, frameFreq, dh, os, marker, numfram, ctr);
		 //downleft
		 process(img, px - 1, py + 1, fillColor, tolerance, frameFreq, dh, os, marker, numfram, ctr);
		 //down 
		 process(img, px , py + 1, fillColor, tolerance, frameFreq, dh, os, marker, numfram, ctr);
...
template <template <class T> class OrderingStructure>
void filler::process(PNG& img, int x, int y, colorPicker& fillColor,
                       double tolerance, int frameFreq, animation& dh, 
					   OrderingStructure<pair<int,int>>& os,
					   vector<vector<bool>>& marker, int& numfram, HSLAPixel& ctr)
{
	//check if x,y valid
	//std::cout<<"width is: "<< img.width()<< "\n";
	//std::cout<<"height is:"<< img.height()<< "\n";
	// if(x == 82 && y == 79){
		// std::cout<<"x is: "<< x<< "\n";
		// std::cout<<"y is: "<< y<< "\n";
		// std::cout<<" "<< "\n";
	...
	if(! (x<0 || x >= (int)img.width() || y < 0 || y >= (int)img.height())){
		//std::cout<<"I'm in "<< "\n";
		if( marker[x][y] == false && ctr.dist(*(img.getPixel(x,y))) <= tolerance){
			//std::cout<<"2 "<< "\n";
			*img.getPixel(x,y) = fillColor(x,y);
